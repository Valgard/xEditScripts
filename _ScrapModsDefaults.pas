...
unit userscript;
    ...
    var
        ...
        bRunScript, bWeapons, bArmors, bNPCs, bOverwrite, bVerbose, bDebug, bChangeMessage : boolean;
    ...
    begin
        ...
        try
            ...
            frm.Caption := wbGameName + ' Scrap Mods Defaults' + scriptVersion;
            frm.Width := 320;
    ...
    procedure ChangeMessage(master, plugin, element : TObject);
    begin
      if not bChangeMessage then begin
          bChangeMessage := true;
          ...
          AddMessage('Change ' + ShortName(element));
      end;
    end;
    ...
    var
        ...
        elementCOBJ, elementOMOD, elementMISC, elementDESC, element : IInterface;
    ...
    begin
        ...
        AddMessage('================================================================================');
        AddMessage('=  Remove components from default mods                                         =');
        AddMessage('================================================================================');
        AddMessage(' ');
        ...
        for k := 0 to Pred(slMasterFilename.Count) do begin
            ...
            for i := 0 to Pred(ElementCount(masterCOBJ)) do begin
                ...
                elementDESC := ElementByPath(elementOMOD, 'DESC');
                if bDebug then begin
                    AddMessage('[DEBUG] {5} elementDESC: ' + FullPath(elementDESC));
                    AddMessage('[DEBUG] {5} elementDESC Value: ' + GetElementEditValues(elementOMOD, 'DESC'));
                end;

                if not SameText(GetElementEditValues(elementOMOD, 'DESC'), 'Standard.') then begin
                    if bVerbose then begin
                        AddMessage('Skip ' +  ShortName(elementCOBJ) + ' [no standard mod]');
                    end;
                    if bDebug then begin
                        AddMessage(' ');
                    end;
                ...
                ChangeMessage(master, plugin, elementCOBJ);

                element := RecordByFormID(plugin, FixedFormID(elementCOBJ), false);
                if bDebug then begin
                    AddMessage('[DEBUG] {7} element: ' + FullPath(element));
                end;

                if not Assigned(element) then begin
                    ChangeMessage(master, plugin, elementCOBJ);
                    AddMessage('    Add to plugin');
                    element := wbCopyElementToFile(elementCOBJ, plugin, false, true);
                    if bDebug then begin
                        AddMessage('[DEBUG] {7.1} element: ' + FullPath(element));
                    end;
                    if not Assigned(element) then begin
                ...
                if FullPath(element) = FullPath(elementCOBJ) then begin
                    ChangeMessage(master, plugin, elementCOBJ);
                    AddMessage('    Add to plugin');
                    element := wbCopyElementToFile(elementCOBJ, plugin, false, true);
                    if bDebug then begin
                        AddMessage('[DEBUG] {7.2} element: ' + FullPath(element));
                    end;
                    if not Assigned(element) then begin
                        // MessageBeep(MB_ICONHAND);
                        AddMessage('[ERROR] Can''t copy base record as new');
                ...
                if bDebug then begin
                    AddMessage('[DEBUG] {7.3} file name plugin: ' + GetFileName(GetFile(plugin)));
                    AddMessage('[DEBUG] {7.3} file name element: ' + GetFileName(GetFile(element)));
                end;
                if GetFileName(GetFile(plugin)) <> GetFileName(GetFile(element)) then begin
                    ...
                    element := wbCopyElementToFile(elementCOBJ, plugin, false, true);
                    if bDebug then begin
                        AddMessage('[DEBUG] {7.3} element: ' + FullPath(element));
                    end;
                    if not Assigned(element) then begin
                        // MessageBeep(MB_ICONHAND);
                        AddMessage('[ERROR] Can''t copy base record as new');
                ...
                componentsNew := ElementByPath(element, 'FVPA');
                if bDebug then begin
                    AddMessage('[DEBUG] {8} element components: ' + FullPath(componentsNew));
                    AddMessage('[DEBUG] {8} element components count: ' + IntToStr(ElementCount(componentsNew)));
                end;

                AddMessage('    Remove components');
                Remove(componentsNew);
                AddMessage(' ');
            end;
        end;
    end;
